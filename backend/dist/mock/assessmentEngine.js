...
function buildSuggestions(dim, score) {
    const out = [];
...
export function mapQuestionsForClient(questions) {
    return questions.map((q) => ({
        ...q,
        options: optionsWithIds(q),
    }));
}
export function evaluateAssessment(questions, body) {
    const answers = body?.answers;
    if (!answers?.length) {
        throw new Error('answers is required');
    }
    const qMap = new Map(questions.map((q) => [q.id, q]));
    const dims = ['info', 'finance', 'psych'];
    const actual = { info: 0, finance: 0, psych: 0 };
    const max = { info: 0, finance: 0, psych: 0 };
    ...
    const suggestions = Array.from(new Set(dims.flatMap((d) => buildSuggestions(d, normalized[d]))));
    return {
        score: total,
        level,
        explanation,
