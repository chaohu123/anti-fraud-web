...
CREATE TABLE `af_user` (
  ...
  `avatar_url`    LONGTEXT     NULL     COMMENT '用户头像URL（base64或URL）',
...
-- ------------------------------------------------------------
-- 知识页轮播图表（管理员上传图片 URL 或本地上传）
-- ------------------------------------------------------------
DROP TABLE IF EXISTS `af_carousel`;
CREATE TABLE `af_carousel` (
  `id`          BIGINT       NOT NULL AUTO_INCREMENT COMMENT '轮播项ID',
  `image_url`   VARCHAR(1024) NOT NULL COMMENT '图片地址（URL 或 /uploads/... 路径）',
  `title`       VARCHAR(100) NULL     COMMENT '标题（可选）',
  ...
  KEY `idx_carousel_enable` (`enable_flag`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='知识页轮播图';
