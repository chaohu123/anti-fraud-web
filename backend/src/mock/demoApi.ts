...
type CaseRow = (typeof casesJson)[number];
type KnowledgeRow = (typeof knowledgeJson)[number];
type QuestionRow = (typeof questionsJson)[number];

const MOCK_DEMO_PASSWORD = 'demo123';

let nextUserId = 1000;
const registeredUsers = new Map<string, { id: number; password: string; nickname: string }>();
...
function toAdminCaseRow(c: CaseRow) {
...
function toAdminKnowledgeRow(k: KnowledgeRow) {
  return {
    ...k,
    contentType: 'ARTICLE',
    status: 'PUBLISHED',
...
function toAdminRiskQuestionRow(q: QuestionRow, idx: number) {
  return {
    id: q.id ?? idx + 1,
...
function filterCases(rows: ReturnType<typeof toAdminCaseRow>[], req: Request) {
  let list = [...rows];
  const { keyword, type, answer, difficulty } = req.query as Record<string, string | undefined>;
  if (keyword) {
    const k = keyword.toLowerCase();
    list = list.filter((r) => r.content.toLowerCase().includes(k) || r.hint.toLowerCase().includes(k));
  }
  if (type) list = list.filter((r) => (r.type || '').toUpperCase() === type.toUpperCase());
  if (answer) list = list.filter((r) => (r.answer || '').toUpperCase() === answer.toUpperCase());
  if (difficulty) list = list.filter((r) => (r.level || '').toUpperCase() === difficulty.toUpperCase());
  return list;
}
...
export function attachDemoRoutes(app: Express) {
  const cases = casesJson as unknown as CaseRow[];
  const knowledge = knowledgeJson as unknown as KnowledgeRow[];
  const questions = questionsJson as unknown as QuestionRow[];
  const carousel = [...(carouselJson as unknown as any[])];
  const achievements = [...(achievementsJson as unknown as any[])];
  const adminUsers = [...(adminUsersJson as unknown as any[])];
  ...
  app.get('/api/knowledge', (req, res) => {
    ...
    if (q) data = data.filter((k) => k.title.includes(q) || k.summary.includes(q));
    res.json(ok(data));
  });
  ...
  app.post('/api/admin/carousel', (req, res) => {
    ...
    const id = Math.max(0, ...carousel.map((c: any) => c.id || 0)) + 1;
    carousel.push({
      id,
  ...
  app.put('/api/admin/carousel/:id', (req, res) => {
    const id = Number(req.params.id);
    const idx = carousel.findIndex((c: any) => c.id === id);
    if (idx >= 0) Object.assign(carousel[idx], req.body);
    res.json(ok(null));
  });
  ...
  app.delete('/api/admin/carousel/:id', (req, res) => {
    const id = Number(req.params.id);
    const idx = carousel.findIndex((c: any) => c.id === id);
    if (idx >= 0) carousel.splice(idx, 1);
    res.json(ok(null));
  });
